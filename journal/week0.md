...
first run:
```
aws sns create-topi --name billing-alarm, it will outpout:
{
...
```
Inside alarm actions is the sns topic created before. Next run the command 
```
aws cloudwatch put-metric-alarm --cli-input-json file://aws/json/alarm-config.json.
```
Now in AWS, CloudWatch inside alarms, the alarm can be seen.
