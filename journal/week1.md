...
- For using docker compose, I had to pull the whole project from github. I also installed docker compose pluging using 1)``` mkdir -p ~/.docker/cli-plugins/``` (create a folder for the plugin) 2)```curl -SL https://github.com/docker/compose/releases/download/v2.26.1/docker-compose-linux-x86_64 -o ~/.docker/cli-plugins/docker-compose ``` (download the docker compose binary) 3) ``` chmod +x ~/.docker/cli-plugins/docker-compose ``` (made the file executable) 4) run command ``` docker compose up -d ```.
- OBSERVATION. As I didn't change "image: frontend-react-js-image" from docker-compose.yml to have the same naming with the image I pulled from repository, when running docker compose up I will create a new image. Also in docker compose I had to change FRONTEND_URL: "http://localhost:3000" and BACKEND_URL: "http://localhost:4567" to be IP offered by AWS for EC2 instead of localhost. Also added in dockerfile for the frontend the following "COPY --from=builder /frontend-react-js ." as I was getting errors because can't find package.json, adding that line now copies all the files needed from system. Then run ```docker compose up -d --build``` to build again.
- Observation2. Since 2021, GitHub does not accept your account password for Git operations in the terminal for security reasons. Instead, you need to provide a Personal Access Token (PAT). I couldn't push changes because of this, when trying to create a new branch specific for EC2. I had to generate an Token, and add user the token when prompted.
